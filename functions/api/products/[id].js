...
export async function onRequestGet(context) {
  ...
  const product = await env.DB.prepare(
    "SELECT id, category_id, brand, gender, sku, name, description, price, retail_price, images, videos, details, size_chart, is_upcoming, available_at, created_at FROM products WHERE id = ? AND deleted_at IS NULL"
  )
    .bind(id)
    .first();
